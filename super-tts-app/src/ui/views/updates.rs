...
const INSTALL_SH_URL: &str =
    "https://raw.githubusercontent.com/jorge-menjivar/super-tts/main/install.sh";
    ...
    match run.phase {
        RunPhase::Done => {
            ...
            if run.completed_components.iter().any(|c| c == "app") {
                col = col.push(
                    row![
                        text::body("Restart Super TTS to finish the update"),
...
pub(crate) fn header_badge(app: &AppModel) -> Option<Element<'_, Message>> {
    ...
    Some(rounded_tooltip(
        badge,
        text::body("A new Super TTS version is ready to install — open the Updates page"),
