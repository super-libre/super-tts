...
pub(super) fn download_card<'a>(
    app: &'a AppModel,
    entry: &'a super_tts_shared::registry::RegistryBackend,
) -> Element<'a, Message> {
    let spacing = cosmic::theme::spacing();
...
pub(super) fn phase_label(p: super_tts_shared::registry::events::InstallPhase) -> &'static str {
    use super_tts_shared::registry::events::InstallPhase;
    match p {
