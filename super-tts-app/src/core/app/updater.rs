...
use super_tts_shared::models::self_update::InstallerAsset;

/// Mirror of super-tts-install's progress.rs wire shapes. The golden tests
...
async fn drive(asset: &InstallerAsset, target_tag: &str, tx: &mut Tx) -> Result<(), String> {
    let base = super_tts_shared::paths::cache_dir().join("self-update");
...
/// `super_tts_registry_types::verify::random_hex_suffix`, the same
/// unpredictable-name pattern `super-tts-install`'s own `StagingGuard` uses
...
fn installer_run_paths(base: &Path, asset_name: &str) -> (PathBuf, PathBuf) {
    let name = format!(
        "{}-{}",
        std::process::id(),
        super_tts_registry_types::verify::random_hex_suffix(8)
    );
...
/// Create `dir` with mode `0700` (matching `super-tts-install`'s own staging
...
/// `super_tts_registry_types::verify::sha256_matches`). On mismatch, deletes
...
async fn verify_installer_checksum(bin: &Path, expected_hex: &str) -> Result<(), String> {
    ...
    let actual = tokio::task::spawn_blocking(move || {
        super_tts_registry_types::verify::file_sha256_hex(&path)
    })
    .await
    .map_err(|e| format!("checksum task panicked: {e}"))?
    .map_err(|e| format!("hash {}: {e}", bin.display()))?;

    if super_tts_registry_types::verify::sha256_matches(&actual, expected_hex) {
        return Ok(());
    }
...
/// Mirrors `super-tts-install`'s `download_to_file` chunk loop, including its
...
async fn download_installer(url: &str, dest: &Path, tx: &mut Tx) -> Result<(), String> {
    ...
    // truncation hazard `super-tts-install::download::download_to_file`
    // guards against.
    file.flush()
        .await
...
mod tests {
    ...
    fn parses_installer_golden_lines() {
        let lines = [
            r#"{"event":"phase","phase":"download","message":"downloading super-tts-x86_64-unknown-linux-gnu-beta.tar.gz"}"#,
    ...
    // emission, mirroring `super-tts-install::download::download_to_file`'s
