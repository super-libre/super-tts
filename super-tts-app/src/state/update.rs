...
pub struct UpdateRun {
    ...
    /// lives in `super-tts-install`'s `InstallError::code`). `None` for a
...
mod tests {
    ...
    fn status(update_available: bool) -> SelfUpdateStatus {
        SelfUpdateStatus {
            ...
            installer_asset: Some(InstallerAsset {
                name: "super-tts-installer".to_string(),
    ...
    /// "Restart Super TTS" CTA alive through the refetch that reports no
    /// update.
    #[test]
