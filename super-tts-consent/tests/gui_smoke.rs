...
//!
//! ```bash
//! cargo test -p super-tts-consent -- --ignored --nocapture
//! ```
//!
//! What's covered:
//!
...
const HELPER_BIN: &str = env!("CARGO_BIN_EXE_super-tts-consent");
...
fn renders_and_decides() {
    ...
    let mut child = Command::new(HELPER_BIN)
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn super-tts-consent");
...
fn surface_survives_compositor_handshake() {
    ...
    let log_path: PathBuf =
        std::env::temp_dir().join(format!("tts-consent-smoke-{}.log", std::process::id()));
