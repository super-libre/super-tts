...
use super_tts_shared::daemon::session;
...
impl SuperTtsApplet {
    ...
    fn daemon_connected(&mut self) -> cosmic_app::Task<Message> {
        ...
        // (`run_widget_subscription` in super-tts-shared owns its own
    ...
    fn launch_app() -> cosmic_app::Task<Message> {
        // `Command::new("super-tts-app")` already searches `PATH`, then fall
        ...
        let launch_attempts = [
            "super-tts-app",                // System PATH
            "/usr/local/bin/super-tts-app", // Local install
            "/usr/bin/super-tts-app",       // System install
        ];

        for command in launch_attempts {
            if Self::spawn_detached(&mut std::process::Command::new(command)).is_ok() {
                info!("Successfully launched Super TTS app with command: {command}");
                return cosmic_app::Task::none();
            }
        }

        warn!("Failed to launch Super TTS app - tried all common locations");
        cosmic_app::Task::none()
    }
