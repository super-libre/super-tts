...
//! CLI integration test: `super-tts-cli record` toggle dispatch.
//!
...
//!
//! ```bash
//! cargo test -p super-tts-cli --test cmd_record_toggle -- --ignored --nocapture
//! ```
...
const DAEMON_BIN: &str = env!("CARGO_BIN_EXE_super-tts-cli");

/// Locate the daemon binary next to the CLI binary's target dir.
/// `cargo test -p super-tts-cli` builds the daemon as a workspace
...
fn spawn_daemon() -> (DaemonGuard, PathBuf) {
    ...
    let unique = format!("tts-cli-toggle-{}-{}", std::process::id(), next_uniq());
...
fn run_cli(socket: &Path, args: &[&str]) -> (i32, String, String) {
    ...
    let mut child = Command::new(cli_bin)
        .env("SUPER_TTS_AUTO_APPROVE", "1")
        .env("SUPER_TTS_HTTP_SOCKET", socket)
        .args(args)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
