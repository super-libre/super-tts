...
//!
//! Backend secrets are stored with service name "super-tts" under per-backend
...
//! `(super-tts, tts-sessions)` — see `daemon::http::internal::auth::tokens::TokenStore` and
...
const SERVICE_NAME: &str = "super-tts";
...
pub const SESSIONS_KEY: &str = "tts-sessions";
...
/// when running under tests or with `SUPER_TTS_KEYRING_MOCK` set.
///
...
/// Whether `SUPER_TTS_KEYRING_MOCK` requests the in-memory store. Honored only
...
fn keyring_mock_env_set() -> bool {
    std::env::var_os("SUPER_TTS_KEYRING_MOCK").is_some()
}
