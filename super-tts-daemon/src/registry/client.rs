...
pub const DEFAULT_URL: &str = "https://jorge-menjivar.github.io/super-tts/index.json";
...
impl Client {
    ...
    pub fn from_env() -> Self {
        let url = match std::env::var("SUPER_TTS_REGISTRY_URL") {
            Ok(v) if crate::registry::accept_base_url(&v) => v,
            Ok(v) => {
                log::warn!("ignoring insecure SUPER_TTS_REGISTRY_URL={v:?}; using {DEFAULT_URL}");
                DEFAULT_URL.into()
            }
            Err(_) => DEFAULT_URL.into(),
        };
        let cache_dir = super_tts_shared::paths::cache_dir();
...
fn persist_to_disk(
    ...
    super_tts_registry_types::fs::write_atomic(cache_path, &serde_json::to_vec(&file)?)?;
    Ok(())
}
