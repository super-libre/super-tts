        ...
        check_min_client(CLIENT_VERSION, &index.min_client)
    {
        log::warn!(
            "registry index requires client >= {min_client}, but this daemon is \
             {client}; newer backends may fail to install or run — please update Super TTS"
        );
    }
}
...
/// `SUPER_TTS_REGISTRY_URL` — is sanitized here rather than failing the
/// whole index.
///
...
/// ([`super_tts_registry_types::backend_id::is_valid`]) rather than to
...
pub fn retain_safe_backends(index: &mut Index) {
    use super_tts_shared::registry::{is_safe_component, is_safe_relative_path};
    for b in &mut index.backends {
        if let Some(id) = b.backend_id.as_deref()
            && !super_tts_registry_types::backend_id::is_valid(id)
        {
            log::warn!(
