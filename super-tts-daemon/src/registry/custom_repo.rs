...
use super_tts_registry_types::manifest::{Kind, Manifest, ManifestError};
use super_tts_registry_types::verify::MAX_MANIFEST_BYTES;
use thiserror::Error;
...
pub async fn resolve(
    ...
    if !super_tts_shared::registry::is_safe_component(&id) {
...
/// `source = github.com/jorge-menjivar/super-tts/openai` to overwrite the
...
// The manifest types are the canonical `super_tts_registry_types::manifest`
...
mod tests {
    ...
    fn source_under_a_different_repo_is_rejected_as_spoof() {
        let repo = RepoRef::parse("github.com/a/b").unwrap();
        // A repo at `a/b` claiming an identity owned by `jorge-menjivar/super-tts`.
        let err = ensure_source_matches_repo("github.com/jorge-menjivar/super-tts/openai", &repo)
            .unwrap_err();
