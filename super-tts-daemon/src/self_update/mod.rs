...
pub(crate) const REPO: &str = "github.com/jorge-menjivar/super-tts";
...
fn effective_beta_optin_for(current: &str, optin: UpdateBetaOptIn) -> bool {
    match optin {
        ...
        UpdateBetaOptIn::Auto => super_tts_registry_types::version::parse_version(current)
...
pub(crate) fn select_candidate(
    ...
    releases
        ...
        .filter_map(|r| super_tts_registry_types::version::parse_version(&r.tag).map(|v| (v, r)))
...
fn find_installer_asset<'a>(release: &'a Release, triple: &str) -> Option<&'a ReleaseAsset> {
    let want = format!("super-tts-install-{triple}");
...
async fn resolve_installer_asset(
    ...
    let bytes = match client
        .download(
            ...
            super_tts_registry_types::verify::MAX_SHA256SUMS_BYTES,
        )
        .await
    {
        Ok(b) => b,
    ...
    let sums = super_tts_registry_types::verify::parse_sha256sums(&text);
        ...
        let new_checker_state = match result {
            Ok(releases) => {
                ...
                let update_available = candidate.is_some_and(|c| {
                    super_tts_registry_types::version::update_available(CURRENT_VERSION, &c.tag)
                });
...
mod tests {
    ...
    fn find_installer_asset_picked_by_exact_name() {
        ...
        r.assets = vec![
            ReleaseAsset {
                name: "super-tts-x86_64-unknown-linux-gnu.tar.gz".into(),
                download_url: "https://dl/t".into(),
                size: 1,
            },
            ReleaseAsset {
                name: "super-tts-install-x86_64-unknown-linux-gnu".into(),
    ...
    fn find_installer_asset_does_not_cross_match_other_triples() {
        ...
        r.assets = vec![
            ...
            ReleaseAsset {
                name: "super-tts-install-aarch64-unknown-linux-gnu".into(),
    ...
    async fn resolve_installer_asset_populates_sha256_from_sums_listing() {
        ...
        s.mock("GET", "/sums")
            .with_status(200)
            .with_body(format!(
                "{target_digest}  super-tts-install-x86_64-unknown-linux-gnu\n\
    ...
    async fn resolve_installer_asset_none_when_entry_has_malformed_hex() {
        ...
        s.mock("GET", "/sums")
            .with_status(200)
            .with_body("not-a-real-digest  super-tts-install-x86_64-unknown-linux-gnu\n")
            .create_async()
            .await;
        let mut r = rel("v0.3.0", ReleaseKind::Published);
        r.assets = vec![
    ...
    async fn run_check_success_and_failure_paths() {
        crate::install_crypto_provider();
        let mut s = mockito::Server::new_async().await;
        s.mock("GET", "/repos/jorge-menjivar/super-tts/releases?per_page=100")
            .with_status(200)
            .with_body(
                r#"[{"tag_name":"v99.0.0","prerelease":false,"assets":[
            {"name":"super-tts-install-x86_64-unknown-linux-gnu","browser_download_url":"https://dl/i","size":5}]}]"#,
            )
            .create_async()
            .await;
        let gh = super_tts_forge::Github::new(s.url(), None);
    ...
    async fn run_check_populates_installer_asset_end_to_end() {
        ...
        s.mock("GET", "/sums")
            .with_status(200)
            .with_body(format!(
                "{digest}  super-tts-install-x86_64-unknown-linux-gnu\n"
            ))
            .create_async()
            .await;
        s.mock("GET", "/repos/jorge-menjivar/super-tts/releases?per_page=100")
            ...
            .with_body(format!(
                ...
                {{"name":"super-tts-install-x86_64-unknown-linux-gnu","browser_download_url":"https://dl/i","size":42}},
        ...
        assert_eq!(asset.name, "super-tts-install-x86_64-unknown-linux-gnu");
        assert_eq!(asset.url, "https://dl/i");
