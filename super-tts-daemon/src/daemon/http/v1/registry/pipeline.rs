...
pub(super) fn spawn_install_pipeline(
    daemon: Arc<crate::daemon::types::SuperTTSDaemon>,
    inflight: Arc<ParkingRwLock<HashSet<String>>>,
    ...
    tokio::spawn(async move {
        ...
        let cache_dir = super_tts_shared::paths::cache_dir().join("install");
        ...
        let pipeline = crate::registry::install::Pipeline {
            ...
            http: super_tts_forge::http::download_client(),
            on_progress: Arc::new(move |phase, bytes: Option<(u64, Option<u64>)>| {
                use super_tts_shared::registry::events::{InstallPhase, RegistryEvent};
