...
/// persistence.
pub(crate) const ALLOW_NO_KEYRING_ENV: &str = "SUPER_TTS_ALLOW_NO_KEYRING";
...
/// back to the system keyring under `(super-tts, tts-sessions)` so a
...
impl TokenStore {
    ...
    pub(crate) fn load_persisted() -> anyhow::Result<Self> {
        ...
        let blob = match crate::keyring::get_sessions_blob() {
            ...
            Err(e) => {
                error!(
                    "System keyring is unavailable or missing ({e}). Super TTS requires \
