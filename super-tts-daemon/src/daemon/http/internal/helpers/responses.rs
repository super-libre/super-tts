...
/// The auth scope catalog now lives in `super-tts-shared` so the daemon and the
...
/// `super_tts_shared::daemon::scopes`.
pub(crate) use super_tts_shared::daemon::scopes::is_known_scope;
