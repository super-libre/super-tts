...
//! `$XDG_RUNTIME_DIR/tts/super-tts-http.sock`, and waits for a shutdown
//! signal. Client commands (`record`, `status`, etc.) live in
//! `super-tts-cli`, which talks to the daemon over HTTP.
...
/// `SUPER_TTS_HTTP_SOCKET` override (tests use this to bind a unique socket
...
async fn spawn_http_listener(daemon: &SuperTTSDaemon) -> Result<tokio::task::JoinHandle<()>> {
    let http_socket_path = super_tts_shared::validation::get_http_socket_path();
...
pub async fn run() -> Result<()> {
    ...
    super_tts_shared::logging::init_with(if verbose {
    ...
    // Defensive: stop any `super-tts-backend-*` --user units left behind by
