...
const PACKAGING: &[&str] = &["super-tts-daemon/systemd/super-tts.service", "justfile"];
...
fn every_shipped_execstart_parses() {
    ...
    for rel in PACKAGING {
        ...
        for args in execstart_args(&text) {
            let argv: Vec<String> = std::iter::once("super-tts-daemon".to_string())
...
fn a_bare_execstart_has_no_arguments() {
    ...
    assert_eq!(execstart_args("ExecStart=super-tts-daemon\n"), no_args);
