...
pub struct DiscoveredBackend {
    ...
    /// Declared secrets the backend expects as `x-tts-secret-*` headers.
    pub secrets: Vec<Secret>,
    /// Declared options the backend accepts as `x-tts-option-*` headers.
    pub options: Vec<Opt>,
...
fn dedup_sources(
    ...
    for (source, mut group) in groups {
        group.sort_by(|a, b| {
            let av = super_tts_registry_types::version::parse_version(&a.version);
            let bv = super_tts_registry_types::version::parse_version(&b.version);
            bv.cmp(&av)
...
/// backend first (`SuperTTSDaemon::active_backend_source`); an empty `source`
...
pub fn default_backends_dir() -> PathBuf {
    super_tts_shared::paths::data_dir().join("backends")
}
