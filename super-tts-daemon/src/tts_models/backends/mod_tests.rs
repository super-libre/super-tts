...
fn scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir()
        .join("super-tts-backend-discovery")
        .join(name);
...
"#,
    ...
    let oai = backends
        .iter()
        .find(|b| b.source == "github.com/super-tts/openai")
...
fn missing_dir_is_empty() {
    let path = std::env::temp_dir().join("super-tts-backend-discovery/does-not-exist");
...
/// the daemon persists in `config.synthesis.active_backend`. A trailing
...
fn dir_name_returns_final_component() {
    ...
    fn fake_backend(dir: PathBuf) -> DiscoveredBackend {
        DiscoveredBackend {
            dir,
            source: "github.com/super-tts/openai".to_string(),
            id: None,
...
fn distinct_sources_resolve_to_the_right_backend() {
    ...
    write_backend(
        &root,
        "openai",
        "github.com/jorge-menjivar/super-tts/openai",
        "OpenAI",
    );
    write_backend(
        &root,
        "mistral",
        "github.com/jorge-menjivar/super-tts/mistral",
        "Mistral",
    );
    write_backend(
        &root,
