...
pub struct AllowlistHooks {
    ...
    /// [`WasmBackend::permit_loopback_egress`](crate::tts_models::wasm::WasmBackend::permit_loopback_egress).
...
impl WasiHttpHooks for AllowlistHooks {
    fn send_request(
        ...
        let port = request.uri().port_u16().unwrap_or_else(|| {
            crate::tts_models::backends::base_url::default_port(request.uri().scheme_str())
        });
