...
//! Host implementation of the outgoing `super-tts:realtime/ws` interface.
//!
//! A realtime backend imports `super-tts:realtime/ws` to open a WebSocket to
...
use crate::tts_models::wasm::host::{Host, check_host_allowed};
...
impl self::super_tts::realtime::ws::Host for Host {
    async fn connect(
        &mut self,
        url: String,
        ...
        let port = uri.port_u16().unwrap_or_else(|| {
            crate::tts_models::backends::base_url::default_port(uri.scheme_str())
        });
...
impl self::super_tts::realtime::ws::HostWsStream for Host {
    async fn send_text(
        &mut self,
...
#[allow(unknown_lints, clippy::unused_async_trait_impl)]
impl self::super_tts::realtime::ws::HostConsumerStream for Host {
    async fn send_text(
        &mut self,
...
/// Register only the `super-tts:realtime/ws` host import on the linker.
///
...
pub fn add_to_linker(linker: &mut Linker<Host>) -> Result<()> {
    self::super_tts::realtime::ws::add_to_linker::<Host, wasmtime::component::HasSelf<Host>>(
        linker,
        |h| h,
    )?;
    Ok(())
