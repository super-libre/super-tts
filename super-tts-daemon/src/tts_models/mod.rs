...
//! [`synthesize`] defines the common trait the hosts present; [`wasm`] and
...
// from `synthesize`'s trait signature, so the module is always compiled; the
