...
//! in `super-tts-shared`; this test pins the HTTP wiring — that exceeding
...
//! against.
//!
//! Hermetic: `SUPER_TTS_AUTO_APPROVE=1` + `SUPER_TTS_KEYRING_MOCK=1`.
...
/// `super-tts-daemon/src/resource_management/mod.rs`.
...
async fn start_daemon() -> (DaemonGuard, PathBuf) {
    let unique = format!("tts-ratelimit-{}-{}", std::process::id(), next_test_uniq());
