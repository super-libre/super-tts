...
/// `super-tts-install`'s own staging directory (`StagingGuard`) and
/// `super-tts-app`'s self-update download directory, so both draw from the
...
mod tests {
    ...
    fn parses_sha256sums_variants() {
        ...
        let d3 = "3".repeat(64);
        let text =
            format!("{d1}  super-tts-x.tar.gz\n{d2} *binary-mode-file\n{d3}  ./dotslash-file\n");
        let sums = parse_sha256sums(&text);
        assert_eq!(sums.len(), 3);
        assert_eq!(sums[0], (d1, "super-tts-x.tar.gz".into()));
