...
//! Secure path helpers: socket-path construction under `$XDG_RUNTIME_DIR/tts/`.

/// Build a validated runtime path `$XDG_RUNTIME_DIR/tts/<relative>` with
/// path-traversal / prefix / length checks on the runtime dir and a
/// `/tmp/tts/<relative>` fallback. `relative` is a caller-controlled subpath
/// (a bare filename, or e.g. `backends/<name>.sock`) joined after `tts/`.
///
...
pub fn secure_runtime_path(relative: &str) -> std::path::PathBuf {
    let fallback = || std::path::PathBuf::from(format!("/tmp/tts/{relative}"));
...
/// Get the path of the HTTP-protocol Unix socket (`super-tts-http.sock`) — the
/// daemon's sole client-facing listener, which all clients connect to.
///
/// A non-empty `SUPER_TTS_HTTP_SOCKET` overrides the path verbatim (tests use
...
/// path is `$XDG_RUNTIME_DIR/tts/super-tts-http.sock` via [`secure_runtime_path`],
/// which applies the traversal / prefix / length guards.
#[must_use]
pub fn get_http_socket_path() -> std::path::PathBuf {
    if let Some(override_path) = std::env::var_os("SUPER_TTS_HTTP_SOCKET")
        && !override_path.is_empty()
    {
        return std::path::PathBuf::from(override_path);
    }
    secure_runtime_path("super-tts-http.sock")
}
...
mod tests {
    ...
    fn secure_runtime_path_guards_xdg_runtime_dir() {
        ...
        let path = secure_runtime_path("super-tts-http.sock");
        assert!(path.to_string_lossy().contains("super-tts-http.sock"));

        // Path traversal falls back to /tmp/tts/.
        unsafe {
            std::env::set_var("XDG_RUNTIME_DIR", "../../../etc");
        }
        assert_eq!(
            secure_runtime_path("super-tts-http.sock"),
            std::path::PathBuf::from("/tmp/tts/super-tts-http.sock")
        );
    ...
    fn http_socket_path_honors_env_override() {
        ...
        unsafe {
            std::env::set_var("SUPER_TTS_HTTP_SOCKET", "/tmp/tts/custom-run.sock");
        }
        assert_eq!(
            get_http_socket_path(),
            std::path::PathBuf::from("/tmp/tts/custom-run.sock")
        );

        // Empty override is ignored — falls back to the runtime-dir path.
        unsafe {
            std::env::set_var("SUPER_TTS_HTTP_SOCKET", "");
        }
        assert!(
            get_http_socket_path()
                .to_string_lossy()
                .ends_with("super-tts-http.sock")
        );

        unsafe {
            std::env::remove_var("SUPER_TTS_HTTP_SOCKET");
        }
    }
}
