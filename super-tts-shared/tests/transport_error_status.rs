...
fn socket_path(name: &str) -> std::path::PathBuf {
    let path = std::env::temp_dir().join(format!("super-tts-transport-test-{name}.sock"));
...
async fn streaming_endpoints_share_the_same_mapping() {
    ...
    let err = super_tts_shared::daemon::http_client::events_stream(socket.clone(), "token", &["a"])
        .await
        .err()
