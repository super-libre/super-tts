...
selectable via
[`GET /pipeline/{stage}/model/list`](../pipeline/model-list.md) (with
`source: "custom"`) and loadable via
[`POST /pipeline/{stage}/model`](../pipeline/model.md#post-pipelinestagemodel).
...
## `POST /settings/custom_models_dir`

**Request:**

```http
POST /settings/custom_models_dir HTTP/1.1
Host: tts.local
...
After the response, calling
[`GET /pipeline/{stage}/model/list`](../pipeline/model-list.md) reflects
...
```http
GET /settings/custom_models_dir HTTP/1.1
Host: tts.local
Authorization: Bearer tts_…64hex…
```
