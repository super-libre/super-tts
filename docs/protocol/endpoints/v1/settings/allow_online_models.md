...
online model via [`POST /pipeline/{stage}/model`](../pipeline/model.md#post-pipelinestagemodel) are
...
## `POST /settings/allow_online_models`

**Request:**

```http
POST /settings/allow_online_models HTTP/1.1
Host: tts.local
...
```http
GET /settings/allow_online_models HTTP/1.1
Host: tts.local
Authorization: Bearer tts_…64hex…
```
