...
[`GET /settings/audio_theme/list`](./audio_theme/list.md); to actually play the
current theme's cues (e.g. as a preview in a settings UI), use
[`POST /settings/audio_theme/test`](./audio_theme/test.md).
...
## `POST /settings/audio_theme`

**Request:**

```http
POST /settings/audio_theme HTTP/1.1
Host: tts.local
...
|---------|--------|----------|------------------------------------------------------------------------|
| `theme` | string | yes      | One of the names returned by [`GET /settings/audio_theme/list`](./audio_theme/list.md)  |
...
```http
GET /settings/audio_theme HTTP/1.1
Host: tts.local
Authorization: Bearer tts_…64hex…
```
