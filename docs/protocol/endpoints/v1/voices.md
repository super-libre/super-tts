...
value without its prefix, for building paths under `/voices`. `transcript` is
...
```http
POST /voices?label=Ada&transcript=The%20quick%20brown%20fox HTTP/1.1
Authorization: Bearer tts_…64hex…
...
`201`, not `200`: a new resource exists at `/voices/{id}`.
