...
/// Closed error set; `code()` values are the wire contract with the app: the
/// app matches on the code, never on the message, so a variant's code is as
/// public as an endpoint's path and renaming one is a breaking change.
#[derive(Debug, Error)]
...
mod tests {
    ...
    fn error_codes_are_the_documented_closed_set() {
        ...
        let cases: Vec<(InstallError, &str)> = vec![
            ...
            (
                ChecksumMismatch("super-tts-x.tar.gz".into()),
                "checksum_mismatch",
            ),
            (
