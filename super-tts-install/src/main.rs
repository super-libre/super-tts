...
//! Installer and self-updater for Super TTS. One binary serves the curl
...
/// Delegates to the shared helper so this crate and `super-tts-app`'s
...
fn run_interactive_menu(
    ...
    loop {
        eprintln!("=============================================");
        eprintln!("      Super TTS Installation Menu");
        eprintln!("=============================================");
        eprintln!();
...
async fn run(cli: &cli::Cli, reporter: Reporter) -> Result<(), InstallError> {
    ...
    let applet_was_installed = prefix.join("bin/super-tts-cosmic-applet").exists();
