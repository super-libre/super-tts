        ...
        match self {
            ...
            Self::Human => match event {
                ...
                Event::Complete {
                    ...
                    println!("\x1b[0;32m[INFO]\x1b[0m Installed Super TTS {installed_version}");
                }
                Event::Error { message, .. } => {
...
mod tests {
    ...
    // GOLDEN STRINGS — duplicated in super-tts-app's parser tests (Task 7.5).
    // Change one side and the other MUST change with it.
    #[test]
    fn json_events_serialize_to_documented_shape() {
        let ev = Event::Phase {
            phase: Phase::Download,
            message: "downloading super-tts-x86_64-unknown-linux-gnu-beta.tar.gz",
        };
        assert_eq!(
