...
/// Canonical `<host>/<owner>/<repo>` reference for Super TTS itself, used for
/// self-update release lookups.
pub const REPO: &str = "github.com/jorge-menjivar/super-tts";

/// Map a `std::env::consts::ARCH`-shaped string to a Rust target triple.
/// Only the architectures Super TTS ships prebuilt Linux binaries for are
...
/// architectures Super TTS ships prebuilt Linux binaries for are supported.
///
/// # Errors
/// [`InstallError::UnsupportedArch`] on anything else (e.g. `mips`).
...
pub fn tarball_name(triple: &str, tag: &str) -> String {
    ...
    if is_beta {
        format!("super-tts-{triple}-beta.tar.gz")
    } else {
        format!("super-tts-{triple}.tar.gz")
    }
}
...
mod tests {
    ...
    fn tarball_name_follows_channel_of_the_tag() {
        assert_eq!(
            ...
            "super-tts-x86_64-unknown-linux-gnu-beta.tar.gz"
        );
        assert_eq!(
            tarball_name("x86_64-unknown-linux-gnu", "v0.2.2"),
            "super-tts-x86_64-unknown-linux-gnu.tar.gz"
        );
    }
    ...
    async fn resolve_target_happy_path_returns_both_urls() {
        ...
        let releases_json = serde_json::json!([{
            ...
            "assets": [
                {
                    "name": "super-tts-x86_64-unknown-linux-gnu-beta.tar.gz",
    ...
    async fn resolve_target_errors_when_release_is_missing_sha256sums() {
        ...
        let releases_json = serde_json::json!([{
            ...
            "assets": [
                {
                    "name": "super-tts-x86_64-unknown-linux-gnu.tar.gz",
                    "browser_download_url": format!("{base}/tarball"),
                    "size": 10,
                },
            ],
    ...
    fn required_asset_missing_names_the_asset() {
        ...
        let err = required_asset(&r, "super-tts-x86_64-unknown-linux-gnu.tar.gz").unwrap_err();
        match err {
            crate::errors::InstallError::DownloadFailed(msg) => {
                assert!(
                    msg.contains("super-tts-x86_64-unknown-linux-gnu.tar.gz"),
                    "{msg}"
                );
            }
