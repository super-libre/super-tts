...
//! `super_tts_registry_types::verify` (shared with the daemon's self-update
//! installer-checksum lookup — both must stay in lock-step); this module
//! composes them into the install-specific error type.

use std::path::Path;

use super_tts_registry_types::verify::{file_sha256_hex, parse_sha256sums, sha256_matches};
