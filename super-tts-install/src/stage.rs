...
use super_tts_registry_types::verify::{tar_budget_step, tar_entry_unsafe_reason, unpack_cap};
...
/// `<prefix>/bin` already has any `super-tts-*` binary, this is an update —
...
pub fn plan_components(
    ...
    let installed: Vec<String> = std::fs::read_dir(prefix.join("bin"))
        ...
        .collect();
    let is_update = installed.iter().any(|name| name.starts_with("super-tts-"));

    if is_update {
        Components {
            daemon: true,
            app: installed.iter().any(|name| name == "super-tts-app"),
            applet: installed
                .iter()
                .any(|name| name == "super-tts-cosmic-applet"),
        }
    } else {
        Components {
            daemon: true,
...
/// (`super-tts-daemon/src/registry/install.rs`) and the indexer's publish-time
/// validation (`super-tts-indexer/src/assets.rs`) so a tarball that passes
...
/// Every `resources/super-tts-cosmic-applet-*.desktop` file in `staging`,
...
fn applet_desktop_files(staging: &Path) -> Result<Vec<PathBuf>, InstallError> {
    ...
    let mut files: Vec<PathBuf> = std::fs::read_dir(&resources)
        ...
        .filter(|path| {
            path.file_name().and_then(|n| n.to_str()).is_some_and(|n| {
                n.starts_with("super-tts-cosmic-applet-") && n.ends_with(".desktop")
            })
        })
        .collect();
    files.sort();
    if files.is_empty() {
        return Err(InstallError::InstallFailed(format!(
            "staging missing any super-tts-cosmic-applet-*.desktop file under {}",
            resources.display()
        )));
    }
    Ok(files)
...
/// install/update re-stages a copy of itself at `<prefix>/bin/super-tts-install`
...
pub fn build_manifest(
    ...
    let mut entries = vec![ManifestEntry {
        ...
        dest: bin.join("super-tts-install"),
        mode: 0o755,
    }];

    if components.daemon {
        for name in ["super-tts-daemon", "super-tts-cli", "super-tts-consent"] {
        ...
        let wrapper_path = generated_dir.join("tts");
        ...
        entries.push(required_entry(
            staging,
            "systemd/super-tts.service",
            unit_dir.join("super-tts.service"),
            0o644,
        )?);
    }
    ...
    if components.app {
        entries.push(required_entry(
            staging,
            "super-tts-app",
            bin.join("super-tts-app"),
            0o755,
        )?);
        entries.push(required_entry(
            staging,
            "resources/super-tts-app.desktop",
            prefix.join("share/applications/super-tts-app.desktop"),
            0o644,
        )?);
        entries.push(required_entry(
            staging,
            "resources/icons/hicolor/scalable/apps/super-tts-app.svg",
            prefix.join("share/icons/hicolor/scalable/apps/super-tts-app.svg"),
            0o644,
        )?);
        // Metainfo is optional upstream (older tarballs may not carry it):
        // skip the entry rather than fail if it's absent.
        let metainfo = staging.join("resources/super-tts-app.metainfo.xml");
        if metainfo.exists() {
            entries.push(ManifestEntry {
                source: metainfo,
                dest: prefix.join("share/metainfo/super-tts-app.metainfo.xml"),
                mode: 0o644,
            });
        }
    }
    ...
    if components.applet {
        entries.push(required_entry(
            ...
            bin.join("super-tts-cosmic-applet"),
            0o755,
        )?);
        ...
        entries.push(required_entry(
            staging,
            "resources/icons/hicolor/scalable/apps/super-tts-cosmic-applet.svg",
            prefix.join("share/icons/hicolor/scalable/apps/super-tts-cosmic-applet.svg"),
            0o644,
        )?);
    }
...
mod tests {
    ...
    fn fake_staging_without(missing: &[&str]) -> PathBuf {
        ...
        let files = [
            ...
            "resources/super-tts-app.desktop",
            "resources/icons/hicolor/scalable/apps/super-tts-app.svg",
            "resources/super-tts-app.metainfo.xml",
            "resources/super-tts-cosmic-applet-full.desktop",
            "resources/super-tts-cosmic-applet-left.desktop",
            "resources/super-tts-cosmic-applet-right.desktop",
            "resources/icons/hicolor/scalable/apps/super-tts-cosmic-applet.svg",
        ];
        for f in files {
            if missing
    ...
    fn detection_update_mode_only_updates_whats_present() {
        let prefix = mktree(&["bin/super-tts-daemon", "bin/super-tts-app"]); // helper: temp dir + touch files
    ...
    fn detection_update_mode_only_applet_present() {
        ...
        let prefix = mktree(&["bin/super-tts-cosmic-applet"]);
        let c = plan_components(None, &prefix, true);
        assert!(c.daemon && !c.app && c.applet);
    }
    ...
    fn detection_update_mode_all_three_present() {
        let prefix = mktree(&[
            "bin/super-tts-daemon",
            "bin/super-tts-app",
            "bin/super-tts-cosmic-applet",
        ]);
    ...
    fn explicit_daemon_selects_only_daemon() {
        let prefix = mktree(&["bin/super-tts-app"]);
    ...
    fn explicit_all_selects_everything_regardless_of_existing_state() {
        let prefix = mktree(&["bin/super-tts-daemon"]);
    ...
    fn manifest_covers_selected_components_with_modes() {
        ...
        let dests: Vec<String> = m
            ...
            .collect();
        assert!(dests.contains(&"/usr/local/bin/super-tts-daemon".to_string()));
        assert!(dests.contains(&"/usr/local/bin/tts".to_string()));
        assert!(dests.contains(&"/usr/lib/systemd/user/super-tts.service".to_string()));
        assert!(dests.contains(&"/usr/local/share/applications/super-tts-app.desktop".to_string()));
        assert!(dests.contains(&"/usr/local/bin/super-tts-install".to_string()));
        assert!(!dests.iter().any(|d| d.contains("cosmic-applet")));
        let unit = m
            .entries
            .iter()
            .find(|e| e.dest.ends_with("super-tts.service"))
            .unwrap();
        assert_eq!(unit.mode, 0o644);
        let tts = m
            .entries
            .iter()
            .find(|e| e.dest.ends_with("bin/tts"))
            .unwrap();
        assert_eq!(tts.mode, 0o755);
        // The generated wrapper's staged content is exactly the known script.
        let content = std::fs::read_to_string(&tts.source).unwrap();
        assert!(content.starts_with("#!/bin/bash\n"));
        assert!(content.contains("exec \"/usr/local/bin/super-tts-cli\" \"$@\""));
    }

    #[test]
    fn manifest_missing_required_source_errors_before_escalation() {
        let staging = fake_staging_without(&["super-tts-app"]);
    ...
    fn manifest_errors_when_applet_desktop_glob_is_empty() {
        ...
        // `super-tts-cosmic-applet-*.desktop` glob yields zero entries, the
    ...
    fn manifest_skips_optional_metainfo_when_absent() {
        let staging = fake_staging_without(&["super-tts-app.metainfo.xml"]);
        let m = build_manifest(
            &staging,
            std::path::Path::new("/usr/local"),
            std::path::Path::new("/usr/lib/systemd/user"),
    ...
    fn manifest_self_exe_entry_is_present_with_0755() {
        ...
        let self_entry = m
            .entries
            .iter()
            .find(|e| e.dest.ends_with("bin/super-tts-install"))
    ...
    fn tarball_round_trip() {
        ...
        std::fs::write(tree.join("super-tts-daemon"), b"bin").unwrap();
        std::fs::write(tree.join("systemd/super-tts.service"), b"[Unit]").unwrap();
        ...
        assert_eq!(std::fs::read(out.join("super-tts-daemon")).unwrap(), b"bin");
        assert_eq!(
            std::fs::read(out.join("systemd/super-tts.service")).unwrap(),
            b"[Unit]"
        );
    }
    ...
    fn extract_tarball_enforces_the_decompression_bomb_cap() {
        ...
        let tgz = tmp.join("bomb.tar.gz");
        {
            ...
            header.set_size(super_tts_registry_types::verify::MAX_TARBALL_ENTRY_BYTES + 1);
            header.set_cksum();
            tar.append(&header, std::io::empty()).unwrap();
            tar.into_inner().unwrap().finish().unwrap();
        }
