...
//! `super-tts-indexer` — top-level orchestration.
...
async fn run_build(args: BuildArgs) -> anyhow::Result<()> {
    ...
    let http = super_engine_forge::http::download_client(super_tts_registry_types::Tts::USER_AGENT);
    ...
    for (id, entry) in &registry.0 {
        ...
        let client =
            super_engine_forge::client(entry.forge, super_tts_registry_types::Tts::USER_AGENT);
    ...
    super_tts_registry_types::fs::write_atomic(&args.out, text.as_bytes())
...
fn temp_part_path() -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("tts-idx-{}-{n}.part", std::process::id()))
}
