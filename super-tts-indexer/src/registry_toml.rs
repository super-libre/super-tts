...
pub use super_tts_registry_types::entry::Entry;
...
fn validate_entry(id_key: &str, e: &Entry) -> Result<(), ParseError> {
    ...
    if let Some(sd) = &e.subdir
        && !super_tts_registry_types::is_safe_relative_path(sd)
    {
        return Err(ParseError::Entry {
            id: id_key.into(),
    ...
    match &e.id {
        Some(id) if !super_tts_registry_types::backend_id::is_valid(id) => {
...
mod tests {
    ...
    fn parses_minimal_entry() {
        ...
        assert_eq!(e.repo, "github.com/jorge-menjivar/super-tts");
...
[brand-new]
    ...
    repo  = "github.com/example/super-tts-brand-new"
    forge = "github"
"#;
    ...
    fn rejects_two_entries_sharing_an_id() {
        let text = format!(
            "{NEW_ENTRY}\n[other]\n    id    = \"com.example.brand-new\"\n    repo  = \"github.com/example/super-tts-other\"\n    forge = \"github\"\n"
        );
        let err = Registry::parse(&text).unwrap_err();
